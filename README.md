...
Are you tired of ads popping up and overloaded interfaces on every famous chess online game? Fortunately for you, stack-underflow and I have created the perfect fully local retro-looking chess game to compete against your favourite local chess engine.
...
## Credits
This game is co-authored with stack-underflow, so far I made the interface that he refactored and he added the possibility to bind it to every uci-speaking chess engine. Also we are proud to say that this code is fully human written as well as the textures 😎, so no AI company can claim to own this code in the future.
...
For any OS the installation starts with a "git clone" of the project, or with a "Download ZIP" if you are too lazy to open your terminal, the next step depends on your distribution:
 - Linux : TODO
 - MacOS : Define in the makefile where you have installed the library SFML (SFML_DIR), and of course you have installed macos developping tools.
...
- h=<min> : human play time in minutes (default = 20)
...
# MY Retro CHess♟️(🇫🇷 Version Française)
Toi aussi tu n'en peux plus des pubs pop up et des interfaces surchargées sur les jeux d'échecs en ligne? Heureusement pour toi, avec stack-underflow nous avons créé le jeu d'échecs parfait pour jouer en local avec une interface rétro, dans laquelle tu peux jouer contre ton moteur d'échec local préféré.
...
## Crédits
Ce jeu est co-écrit avec stack-underflow. Pour l'instant j'ai codé l'interface SFML, qu'il a restructuré et il a ajouté la possibilité de la lier avec n'importe quel moteur d'échec qui parle l'Interface Universelle d'Échec (UCI, oui on est fâché avec l'anglais).
...
Pour n'importe quel système d'exploitation l'installation commence avec un "git clone" du projet, ou avec un "Download ZIP" si tu es trop fainéant pour ouvrir un terminal, l'étape suivante dépends de ta distribution:
 - Linux : TODO
 - MacOS : Définir dans le makefile indiquer où vous avez installé la librairie SFML (variable SFML_DIR), et bien sûr avoir installé les outils de développement MacOsx.
...
- h=<min> : cadence du joueur en minutes (défaut = 20)
